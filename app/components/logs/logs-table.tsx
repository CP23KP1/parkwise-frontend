  ...
  const renderCell = (item: LogsRowData, columnKey: keyof LogsRowData) => {
    ...
    switch (columnKey) {
      ...
      case "arrowDirection":
        return (
          <>
            {item.arrowDirection === 'in' ? <p className="text-green-500">เข้า</p> : <p className="text-red-500">ออก</p>}
          </>
        );
      default:
        return cellValue;
  ...
  return (
    <div className="table-container w-72 sm:w-full">
      ...
      >
        ...
        <TableBody emptyContent={"ไม่มีข้อมูลรถยนต์"} items={sortedItems}>
          {(item) => (
            <TableRow key={item.id}>
              {(columnKey) => (
                <TableCell>
                  {renderCell(item, columnKey as keyof LogsRowData)}
                </TableCell>
              )}
            </TableRow>
          )}
